...
const MenuContainer = styled(Card)<MenuProps & { pos?: DOMRect | null }>`
	${({ pos, anchorOrigin, positionOverride, theme, transformOrigin, width }) => {
		const getLeft = () => {
...
export const Menu = forwardRef<HTMLDivElement, MenuProps>(function Menu({ children, ...props }, ref) {
	...
	const clonedChildren = Children.map(children, (child) => {
		if (isValidElement(child) && typeof child.type !== 'string') {
			...
			return cloneElement(
				child as ReactElement<{ padEnd: boolean; padStart: boolean }, string | JSXElementConstructor<unknown>>,
				{
	...
	return props.open
		? createPortal(
				<>
					{!isNested && <Backdrop onClose={handleClickAway} bgColor="transparent" {...props.backdropProps} />}
					<MenuContainer ref={menuRef} pos={position} onClick={handleClick} {...props}>
