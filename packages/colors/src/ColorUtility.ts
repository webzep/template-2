...
export class ColorUtility {
	static rgb = {
		...
		clean: (color: string) => {
			...
			if (parts.length === 4 && (parts[3].includes('%') || Number(parts[3]) > 1)) {
		...
		compose: (parts: (number | string)[]) => {
			if (parts.length < 3) {
				throw new Error('ColorUtility.rgb.compose: Parts must have at least three values.');
			}
		...
		decompose: (color: string): string[] => {
			return ColorUtility.rgb.clean(color).split('(')[1].split(')')[0].split(',');
		},
		...
		isValid: (color: string) => {
			return color.slice(0, 3) === 'rgb' && ColorUtility.rgb.decomposeNumeric(color).length > 2;
		},
		set: {
		...
		validateWithError: (color: string) => {
			if (!ColorUtility.rgb.isValid(color))
				throw new Error('ColorUtility.rgb.validateWithError: Color must be RGB.');
		},
	};
	...
	static hsl = {
		...
		compose: (parts: (number | string)[]) => {
			if (parts.length < 3) {
				throw new Error('ColorUtility.hsl.compose: Parts must have at least three values.');
			}
			const p = parts.map((p) => String(p));
		 ...
		 */
		decompose: (color: string): string[] => {
			return ColorUtility.hsl.clean(color).split('(')[1].split(')')[0].split(',');
		},
		...
		isValid: (color: string) => {
			return color.slice(0, 3) === 'hsl' && ColorUtility.rgb.decompose(color).length > 2;
		},
		set: {
		...
		to: {
			...
			hex: (color: string) => {
				const parts = ColorUtility.hsl.decompose(ColorUtility.hsl.clean(color));
				...
				const f = (n: number) => {
					const k = (n + h / 30) % 12;
					const color = light - j * Math.max(Math.min(k - 3, 9 - k, 1), -1);

					return Math.round(255 * color)
						.toString(16)
						.padStart(2, '0');
				};
				const a = Math.round((ColorUtility.hsl.get.opacity(color) / 1) * 255);
		...
		validateWithError: (color: string) => {
			if (!ColorUtility.hsl.isValid(color))
				throw new Error('ColorUtility.hsl.validateWithError: Color must be HSL.');
		},
	};
	...
	static hex = {
		...
		clean: (color: string) => {
			if (!ColorUtility.hex.isValid(color))
				throw new Error("ColorUtility.hex.clean: Can't clean a non-hex color");
		...
		compose: (parts: string[]) => {
			if (parts.length < 3) {
				throw new Error('ColorUtility.hex.compose: Parts must have at least three values.');
			}
			const p = parts.map((v) => (v.length > 1 ? v : `${v}${v}`).toLowerCase());
		...
		decompose: (color: string) => {
			...
			const chars = ColorUtility.hex.clean(color).replace('#', '').split('');
			const parts = [];
		...
		get: {
			...
			opacityAsNumber: (color: string) => {
				...
				return Number((ColorUtility.hex.to.number(opacity) / 255).toFixed(2));
			},
		...
		to: {
			...
			rgb: (hex: string) => {
				...
				if (parts.length > 3) parts[3] = Number((parts[3] / 255).toFixed(2));
		...
		validateWithError: (color: string) => {
			if (!ColorUtility.hex.isValid(color))
				throw new Error('ColorUtility.hex.validateWithError: Color must be HEX.');
		},
	};
	...
	static css = {
		...
		to: {
			...
			hsl: (color: string) => {
				...
				return ColorUtility.hex.to.hsl(cssColorMap[color as keyof typeof cssColorMap]);
			},
			/** Convert an CSS color to RGB. */
			rgb: (color: string) => {
				ColorUtility.css.validateWithError(color);

				return ColorUtility.hex.to.rgb(cssColorMap[color as keyof typeof cssColorMap]);
			},
		},
		/** Check if a string is an CSS color, throw an error if not. */
		validateWithError: (color: string) => {
			if (!ColorUtility.css.isValid(color))
				throw new Error('ColorUtility.css.validateWithError: Color must be an CSS color.');
		},
	};
	...
	static get = {
		...
		blue: (color: string) => {
			if (ColorUtility.hex.isValid(color)) return ColorUtility.hex.get.blueAsNumber(color);
			else if (ColorUtility.rgb.isValid(color)) return ColorUtility.rgb.get.blue(color);
			else if (ColorUtility.hsl.isValid(color)) return ColorUtility.rgb.get.blue(ColorUtility.hsl.to.rgb(color));
			else if (ColorUtility.css.isValid(color)) return ColorUtility.rgb.get.blue(ColorUtility.css.to.rgb(color));

			throw new Error('ColorUtility.get.blue: Only hex, rgb, hsl and css colors are supported.');
		},
		...
		contrast: (foreground: string, background: string): number => {
			...
			return (Math.max(lumF, lumB) + 0.05) / (Math.min(lumF, lumB) + 0.05);
		},
		/** Get the green of a color. */
		green: (color: string) => {
			if (ColorUtility.hex.isValid(color)) return ColorUtility.hex.get.greenAsNumber(color);
			else if (ColorUtility.rgb.isValid(color)) return ColorUtility.rgb.get.green(color);
			else if (ColorUtility.hsl.isValid(color)) return ColorUtility.rgb.get.green(ColorUtility.hsl.to.rgb(color));
			else if (ColorUtility.css.isValid(color)) return ColorUtility.rgb.get.green(ColorUtility.css.to.rgb(color));

			throw new Error('ColorUtility.get.green: Only hex, rgb, hsl and css colors are supported.');
		},
		/** Get the hue of a color. */
		hue: (color: string) => {
			if (ColorUtility.hex.isValid(color)) return ColorUtility.hsl.get.hue(ColorUtility.hex.to.hsl(color));
			else if (ColorUtility.rgb.isValid(color)) return ColorUtility.hsl.get.hue(ColorUtility.rgb.to.hsl(color));
			else if (ColorUtility.hsl.isValid(color)) return ColorUtility.hsl.get.hue(color);
			else if (ColorUtility.css.isValid(color)) return ColorUtility.hsl.get.hue(ColorUtility.css.to.hsl(color));

			throw new Error('ColorUtility.get.hue: Only hex, rgb, hsl and css colors are supported.');
		},
		/** Get the lightness of a color. */
		lightness: (color: string) => {
			if (ColorUtility.hex.isValid(color)) return ColorUtility.hsl.get.lightness(ColorUtility.hex.to.hsl(color));
			else if (ColorUtility.rgb.isValid(color))
				return ColorUtility.hsl.get.lightness(ColorUtility.rgb.to.hsl(color));
			else if (ColorUtility.hsl.isValid(color)) return ColorUtility.hsl.get.lightness(color);
			else if (ColorUtility.css.isValid(color))
				return ColorUtility.hsl.get.lightness(ColorUtility.css.to.hsl(color));

			throw new Error('ColorUtility.get.lightness: Only hex, rgb, hsl and css colors are supported.');
		},
		...
		opacity: (color: string) => {
			if (ColorUtility.hex.isValid(color)) return ColorUtility.hex.get.opacityAsNumber(color);
			else if (ColorUtility.rgb.isValid(color)) return ColorUtility.rgb.get.opacity(color);
			else if (ColorUtility.hsl.isValid(color)) return ColorUtility.hsl.get.opacity(color);
			else if (ColorUtility.css.isValid(color)) return 1;
			throw new Error('ColorUtility.get.opacity: Only hex, rgb, hsl and css colors are supported.');
		},
		/** Get the red of a color. */
		red: (color: string) => {
			if (ColorUtility.hex.isValid(color)) return ColorUtility.hex.get.redAsNumber(color);
			else if (ColorUtility.rgb.isValid(color)) return ColorUtility.rgb.get.red(color);
			else if (ColorUtility.hsl.isValid(color)) return ColorUtility.rgb.get.red(ColorUtility.hsl.to.rgb(color));
			else if (ColorUtility.css.isValid(color)) return ColorUtility.rgb.get.red(ColorUtility.css.to.rgb(color));

			throw new Error('ColorUtility.get.red: Only hex, rgb, hsl and css colors are supported.');
		},
		/** Get the saturation of a color. */
		saturation: (color: string) => {
			if (ColorUtility.hex.isValid(color)) return ColorUtility.hsl.get.saturation(ColorUtility.hex.to.hsl(color));
			else if (ColorUtility.rgb.isValid(color))
				return ColorUtility.hsl.get.saturation(ColorUtility.rgb.to.hsl(color));
			else if (ColorUtility.hsl.isValid(color)) return ColorUtility.hsl.get.saturation(color);
			else if (ColorUtility.css.isValid(color))
				return ColorUtility.hsl.get.saturation(ColorUtility.css.to.hsl(color));

			throw new Error('ColorUtility.get.saturation: Only hex, rgb, hsl and css colors are supported.');
		},
	};
	...
	static to = {
		...
		hex: (color: string) => {
			if (ColorUtility.hex.isValid(color)) {
				...
				throw new Error('ColorUtility.to.hex: Only hex, rgb, hsl and css colors are supported.');
			}
		},
		...
		hsl: (color: string) => {
			if (ColorUtility.hsl.isValid(color)) {
				...
				throw new Error('ColorUtility.to.hsl: Only hex, rgb, hsl and css colors are supported.');
			}
		},
		...
		rgb: (color: string) => {
			if (ColorUtility.rgb.isValid(color)) {
				...
				throw new Error('ColorUtility.to.rgb: Only hex, rgb, hsl and css colors are supported.');
			}
		},
	};
