...
export const getInitialEntityState = <T extends EntitySchema>(): EntityState<T> => ({
	entities: undefined,
	ids: undefined,
});

export const makeBuilderAddCaseMerge = <T extends AsyncThunk<Array<any>, unknown, null>>( // eslint-disable-line @typescript-eslint/no-explicit-any
	...
	builder.addCase(thunkAction.fulfilled, (state, action) => {
		state[path] = mergeResponseToExistingEntityState(state[path], action.payload);
	});
};
...
const mapResponseToEntityState = <T extends EntitySchema>(response: T[]): EntityState<T> => {
	const responseIds = response?.map((entity) => entity.id) ?? [];
	const newEntities = Object.fromEntries(response?.map((entity) => [entity.id, entity]));
...
export const mergeResponseToExistingEntityState = <T extends EntitySchema>(
	...
	const uniqueEntities = Object.fromEntries(uniqueIds.map((id) => [id, allEntities[id]]));
