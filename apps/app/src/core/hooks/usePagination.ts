...
const applyFilterToEntities = <T>(
	...
	const filteredEntities = Object.values(entityState.entities).filter((entity) =>
...
const applySearchTermToEntities = <T>(
	...
	return entityState.entities
		? Object.values(entityState.entities).filter((entity) => {
				...
				return entityValues.some((entry) => checkIsKeyMatched<T>(entry, term));
		  })
		: [];
};
...
export const usePagination = <T>({
	...
	const [rowExclusions, setRowExclusions] = useState<UsePaginationProps<T>['exclude']>(exclude);
	...
	const pageEntities = searchResults?.slice((page - 1) * pageSize, page * pageSize) ?? [];
	...
	const handleSearchChanged = (term: string) => {
		...
		const filteredEntities = applyFilterToEntities(entityState, rowExclusions);
		const searchResults = applySearchTermToEntities(filteredEntities, searchByKeys, term);
