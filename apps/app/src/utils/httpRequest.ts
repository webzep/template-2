...
const requestWithoutBody =
	(method: HttpMethod) =>
	async <FetchParams = null, ApiResponse = null>(url: string, options: FetchOptions<null, FetchParams> = {}) =>
...
const request = async <Body = null, FetchParams = null, ApiResponse = null>(
	...
	if (options?.params) {
		const parsedParams = Object.fromEntries(
			Object.entries(options.params).map(([key, value]) => [key, JSON.stringify(value)])
		);
